...
describe('Promisify', function () {
  ...
  describe('#callbackAsPromise', function () {
    it('creates a function that make callbacks into promises', function () {
      const anyResult = random.number()
      const method = mock()
      ...
      return Promise.resolve()
        .then(() => expect(promise).to.eventually.equal(anyResult))
        .then(() => expect(promise2).to.eventually.equal(anyResult))
    })
  })
